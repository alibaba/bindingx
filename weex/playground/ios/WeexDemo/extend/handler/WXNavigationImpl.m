...
@interface WXBarButton2 :UIButton
...
{
    if (0 == [param count] || !param[@"url"]) {
        return;
    }
    
    NSString *url = param[@"url"];
    NSString *scheme = [[NSURL URLWithString:url] scheme];
    if (!scheme || [[scheme lowercaseString] isEqualToString:@"http"] || [[scheme lowercaseString] isEqualToString:@"https"]) {
        
        NSString *url = [param objectForKey:@"url"];
        NSURL *currentURL = nil;
        if ([container isKindOfClass:[WXDemoViewController class]]) {
            WXDemoViewController *currentViewController = (WXDemoViewController *)container;
            currentURL = currentViewController.url;
        }
        NSURL *URL = [WXURLResolver getAbsoluteURL:url currentURL:currentURL];
        WXDemoViewController *vc = [WXDemoViewController new];
        vc.url = URL;
        [container presentViewController:[[WXRootViewController alloc] initWithRootViewController:vc] animated:YES completion:nil];
        
        if (success) {
            success(@{@"result":MSG_SUCCESS});
        }
        return;
    }
    else {
...
withContainer:(UIViewController *)container
{
    [container dismissViewControllerAnimated:NO completion:nil];
    if (success) {
        success(@{@"result":MSG_SUCCESS});
    }
}
                      ...
                      withContainer:(UIViewController *)container
{
    ...
    BOOL animated = param[@"animated"] ? [WXConvert BOOL:param[@"animated"]] : YES;
        ...
        withContainer:(UIViewController *)container
{
    if (param[@"title"]) {
        ...
        WXBarButton2 *button = [WXBarButton2 buttonWithType:UIButtonTypeRoundedRect];
...
{
    WXBarButton2 *button = (WXBarButton2 *)sender;
